...
async def test_local_address_real() -> None:
    with trio.socket.socket() as listener:
        ...
        # It's hard to tests local_address properly, because you need multiple
        ...
        # certain we have is 127.0.0.1, so we can't really tests local_address=
        ...
        # tests to make sure the local_address= code doesn't crash.
...
class Scenario(trio.abc.SocketFactory, trio.abc.HostnameResolver):
    ...
    async def getaddrinfo(
        ...
        assert host == b"tests.example.com"
...
async def run_scenario(
    ...
    **kwargs: Any,
    ...
    try:
        stream = await open_tcp_stream("tests.example.com", port, **kwargs)
        assert expect_error == ()
...
async def test_all_fail(autojump_clock: MockClock) -> None:
    ...
    assert RaisesGroup(
        *subexceptions, match="all attempts to connect to tests.example.com:80 failed"
