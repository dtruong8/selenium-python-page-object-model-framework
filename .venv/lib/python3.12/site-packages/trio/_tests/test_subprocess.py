...
async def test_stderr_stdout(background_process: BackgroundProcessType) -> None:
    ...
    # equivalent tests with run_process()
    result = await run_process(
        COPY_STDIN_TO_STDOUT_AND_BACKWARD_TO_STDERR,
        stdin=b"1234",
        capture_stdout=True,
...
def test_waitid_eintr() -> None:
    ...
    # ourselves) but the tests works on all waitid platforms.
    ...
    try:
        ...
        if sleeper.returncode is None:  # pragma: no cover
            ...
            # if the tests passes, wait() will reap the process
            sleeper.kill()
            sleeper.wait()
...
# check=False, so lets have a tests that uses check=True as well
