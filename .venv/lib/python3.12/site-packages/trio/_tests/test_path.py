...
def path(tmp_path: pathlib.Path) -> trio.Path:
    return trio.Path(tmp_path / "tests")
...
async def test_magic() -> None:
    path = trio.Path("tests")

    assert str(path) == "tests"
    assert bytes(path) == b"tests"
...
# upstream python3.8 bug: we should also tests (pathlib.Path, trio.Path), but
...
async def test_forward_methods_rewrap(path: trio.Path, tmp_path: pathlib.Path) -> None:
    ...
    assert with_suffix == tmp_path / "tests.py"
