...
#
# Both present a certificate for "trio-tests-1.example.org".

TRIO_TEST_CA = trustme.CA()
TRIO_TEST_1_CERT = TRIO_TEST_CA.issue_server_cert("trio-tests-1.example.org")
...
# Fixture that gives a raw socket connected to a trio-tests-1 echo server
...
# Fixture that gives a properly set up SSLStream connected to a trio-tests-1
...
async def ssl_echo_server(
    ...
    async with ssl_echo_server_raw(**kwargs) as sock:
        yield SSLStream(sock, client_ctx, server_hostname="trio-tests-1.example.org")
...
class PyOpenSSLEchoStream(Stream):
    def __init__(self, sleeper: None = None) -> None:
        ...
        # need to tests renegotiation support, which means we need to force this
        # to use a lower version where this tests server can trigger
                ...
                while True:
                    ...
                    try:
                        ...
                        if self._pending_cleartext:
                            ...
                            # tests suite, but it's not an interesting omission
                            # so let's pragma it.
...
async def test_PyOpenSSLEchoStream_gives_resource_busy_errors() -> None:
    ...
    # might accidentally do this are during renegotiation, which we tests using
...
def virtual_ssl_echo_server(
    ...
    yield SSLStream(fakesock, client_ctx, server_hostname="trio-tests-1.example.org")
...
def ssl_wrap_pair(
    ...
    *,
    ...
    client_ssl = SSLStream(
        client_transport,
        client_ctx,
        server_hostname="trio-tests-1.example.org",
        **client_kwargs,
    )
...
# Simple smoke tests for handshake/send/receive/shutdown talking to a
...
async def test_ssl_client_basics(client_ctx: SSLContext) -> None:
    ...
    async with ssl_echo_server_raw(expect_fail=True) as sock:
        ...
        s = SSLStream(sock, bad_client_ctx, server_hostname="trio-tests-1.example.org")
        assert not s.server_side
    ...
    async with ssl_echo_server_raw(expect_fail=True) as sock:
        s = SSLStream(sock, client_ctx, server_hostname="trio-tests-2.example.org")
        assert not s.server_side
        with pytest.raises(BrokenResourceError) as excinfo:
            await s.send_all(b"x")
    ...
    with a, b:
        ...
        def client() -> None:
            with client_ctx.wrap_socket(
                a, server_hostname="trio-tests-1.example.org"
            ) as client_sock:
...
async def test_attributes(client_ctx: SSLContext) -> None:
    async with ssl_echo_server_raw(expect_fail=True) as sock:
        ...
        s = SSLStream(sock, good_ctx, server_hostname="trio-tests-1.example.org")
        ...
        assert s.server_hostname == "trio-tests-1.example.org"
...
# Note: this tests fails horribly if we force TLS 1.2 and trigger a
...
#
# In some variants of this tests (maybe only against the java server?) I've
...
async def test_ssl_over_ssl(client_ctx: SSLContext) -> None:
    ...
    client_1 = SSLStream(
        client_0, client_ctx, server_hostname="trio-tests-1.example.org"
    )
    server_1 = SSLStream(server_0, SERVER_CTX, server_side=True)

    client_2 = SSLStream(
        client_1, client_ctx, server_hostname="trio-tests-1.example.org"
    )
...
async def test_ssl_handshake_failure_during_aclose() -> None:
    ...
    async with ssl_echo_server_raw(expect_fail=True) as sock:
        ...
        s = SSLStream(sock, client_ctx, server_hostname="trio-tests-1.example.org")
...
async def test_getpeercert(client_ctx: SSLContext) -> None:
    ...
    assert ("DNS", "trio-tests-1.example.org") in client.getpeercert()["subjectAltName"]
    ...
    async def setup(
        **kwargs: Any,
        ...
        ssl_client = SSLStream(
            transport_client, client_ctx, server_hostname="trio-tests-1.example.org"
        )
