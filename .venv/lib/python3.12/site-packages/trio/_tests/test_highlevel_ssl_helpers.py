...
async def test_open_ssl_over_tcp_stream_and_everything_else(
    ...
    async with trio.open_nursery() as nursery:
        ...
        async with listener:
            ...
            stream = await open_ssl_over_tcp_stream("trio-tests-1.example.org", 80)
            async with stream:
            ...
            stream = await open_ssl_over_tcp_stream(
                "trio-tests-1.example.org", 80, ssl_context=client_ctx
            )
            async with stream:
                assert isinstance(stream, trio.SSLStream)
                assert stream.server_hostname == "trio-tests-1.example.org"
            ...
            stream = await open_ssl_over_tcp_stream(
                "trio-tests-1.example.org",
                80,
                ssl_context=client_ctx,
                https_compatible=True,
                # also, smoke tests happy_eyeballs_delay
