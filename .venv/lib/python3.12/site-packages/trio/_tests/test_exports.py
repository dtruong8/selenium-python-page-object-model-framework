...
def _ensure_mypy_cache_updated() -> None:
    ...
    if not mypy_cache_updated:
        ...
        # but `pytest -k ...` might run just this tests on its own
        result = run(
            [
                "--config-file=",
...
# It doesn't make sense for downstream redistributors to run this tests, since
# they might be using a newer version of Python with additional symbols which
# won't be reflected in trio.socket, and this shouldn't cause downstream tests
