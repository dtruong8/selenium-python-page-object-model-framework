        ...
        async with trio.open_nursery() as nursery:
            ...
            async def route_packet(packet: UDPPacket) -> None:
                while True:
                    ...
                    # I wanted to tests random packet corruption too, but it turns out
...
async def test_invalid_cookie_rejected(autojump_clock: trio.abc.Clock) -> None:
    ...
    with trio.CancelScope() as cscope:
        # the first 11 bytes of ClientHello aren't protected by the cookie, so only tests
            ...
            try:
                ...
                if len(cookie) != 0:
                    ...
                    if offset >= len(payload):
                        ...
                        # payload, and terminate the tests.
...
async def test_openssl_retransmit_doesnt_break_stuff() -> None:
    # can't use autojump_clock here, because the point of the tests is to wait for
...
def test_gc_after_trio_exits() -> None:
    async def main() -> DTLSEndpoint:
        # We use fakenet just to make sure no real sockets can leak out of the tests
        # case - on pypy somehow the socket was outliving the gc_collect_harder call
        # below. Since the tests is just making sure DTLSEndpoint.__del__ doesn't explode
...
async def test_association_replaced_before_handshake_starts() -> None:
    ...
    # This tests shouldn't send any packets
