...
async def test_named_thread() -> None:
    ...
    # tests that you can set a custom name, and that it's reset afterwards
...
async def test_run_in_worker_thread_limiter(
    ...
    # This tests is a bit tricky. The goal is to make sure that if we set
...
async def test_from_thread_check_cancelled() -> None:
    ...
    def f() -> None:
        try:
            from_thread_check_cancelled()
        except _core.Cancelled:  # pragma: no cover, tests failure path
            q.put("Cancelled")
        else:
            q.put("Not Cancelled")
        ev.wait()
    ...
    def f() -> None:  # type: ignore[no-redef] # noqa: F811
        ...
        try:
            ...
            q.put("Cancelled")
        else:  # pragma: no cover, tests failure path
            q.put("Not Cancelled")
...
async def test_reentry_doesnt_deadlock() -> None:
    # Regression tests for issue noticed in GH-2827
    # The failure mode is to hang the whole tests suite, unfortunately.
