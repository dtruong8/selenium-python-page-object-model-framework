...
def test_winerror(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mock.return_value = (12, "tests error")
    with pytest.raises(
        OSError, match=r"^\[WinError 12\] tests error: 'file_1' -> 'file_2'$"
    ) as exc:
        raise_winerror(filename="file_1", filename2="file_2")
    mock.assert_called_once_with()
    mock.reset_mock()
    assert exc.value.winerror == 12
    assert exc.value.strerror == "tests error"
    ...
    with pytest.raises(
        OSError, match=r"^\[WinError 18\] tests error: 'a/file' -> 'b/file'$"
    ) as exc:
...
async def test_readinto_overlapped() -> None:
    ...
    with tempfile.TemporaryDirectory() as tdir:
        ...
        with open(  # noqa: ASYNC101  # This is a tests, synchronous is ok
            tfile, "wb"
        ) as fp:
            fp.write(data)
            fp.flush()
...
def test_forgot_to_register_with_iocp() -> None:
    with pipe_with_overlapped_read() as (write_fp, read_handle):
        ...
        # gets put with the correct tests
        del exc_info
        gc_collect_harder()
