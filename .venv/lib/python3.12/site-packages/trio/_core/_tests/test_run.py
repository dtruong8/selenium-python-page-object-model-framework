...
def not_none(x: T | None) -> T:
    ...
    This is just to satisfy type checkers, if this ever fails the tests is broken.
    """
...
async def test_current_statistics(mock_clock: _core.MockClock) -> None:
    ...
    async with _core.open_nursery() as nursery:
        ...
        # runnable on the next pass), but still useful to at least tests the
...
async def test_cancel_shield_abort() -> None:
    with _core.CancelScope() as outer:
        async with _core.open_nursery() as nursery:
            ...
            # written, without these last few lines, the tests spuriously
...
# Regression tests for https://github.com/python-trio/trio/issues/1175
...
async def test_timekeeping() -> None:
    # probably a good idea to use a real clock for *one* tests anyway...
    TARGET = 1.0
...
# so there's no special code in Trio required to pass this tests, but it's
# still nice to know that it works :-).
#
# Update: it turns out I was right to be nervous! see the next tests...
...
# Similar to previous tests -- if the `ValueError()` gets sent in via 'throw' and
...
def test_TrioToken_run_sync_soon_idempotent_requeue() -> None:
    ...
    # will call it again. Also this is much easier to tests :-)
    record: list[None] = []
...
async def test_TrioToken_run_sync_soon_massive_queue() -> None:
    ...
    # so let's try to make that happen. This is also just a good stress tests
...
async def test_asyncio_function_inside_nursery_does_not_explode() -> None:
    # Regression tests for https://github.com/python-trio/trio/issues/552
...
async def test_nursery_start_with_cancelled_nursery() -> None:
    ...
    # convenient way to get a nursery that we can tests spawning stuff into.
...
def test_contextvar_support() -> None:
    var: contextvars.ContextVar[str] = contextvars.ContextVar("tests")
    var.set("before")
...
async def test_contextvar_multitask() -> None:
    var = contextvars.ContextVar("tests", default="hmmm")
...
async def test_nursery_loose_exception_groups() -> None:
    ...
    async def raise_error() -> NoReturn:
        raise RuntimeError("tests error")

    with pytest.raises(RuntimeError, match="^tests error$"):
        async with _core.open_nursery(strict_exception_groups=False) as nursery:
            nursery.start_soon(raise_error)
    m = Matcher(RuntimeError, match="^tests error$")
...
async def test_cancel_scope_no_cancellederror() -> None:
    ...
    with RaisesGroup(RuntimeError, RuntimeError, match="tests"):
        with _core.CancelScope() as scope:
            scope.cancel()
            raise ExceptionGroup("tests", [RuntimeError(), RuntimeError()])
